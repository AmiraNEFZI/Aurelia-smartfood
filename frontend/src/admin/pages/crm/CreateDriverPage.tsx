...
export function CreateDriverPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setLoading(true)
    try {
      await adminUsersApi.createDriver(form)
      setSuccess(true)
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6 max-w-2xl">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-100">Créer un compte Livreur</h1>
          <p className="text-slate-500 text-sm">Le livreur pourra se connecter et gérer ses livraisons</p>
        </div>
      </div>
      ...
      {success ? (
        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
          ...
          <div className="w-16 h-16 rounded-full bg-orbit-success/15 flex items-center justify-center mx-auto mb-4">
            <span className="text-emerald-400 text-3xl">✓</span>
          </div>
          <h2 className="text-xl font-bold text-slate-100 mb-2">Livreur créé avec succès !</h2>
          <p className="text-slate-500 text-sm">Redirection vers la liste...</p>
        </motion.div>
      ) : (
        <Card>
          <CardHeader title="Informations du livreur" subtitle="Tous les champs marqués * sont obligatoires" />
          <CardBody className="pt-5 border-t border-orbit-border mt-4">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <Input label="Téléphone *" name="phone" type="tel" placeholder="+216 XX XXX XXX"
                prefix={<Phone className="w-3.5 h-3.5" />} value={form.phone} onChange={handleChange} required />
              <Input label="Email *" name="email" type="email" placeholder="livreur@smartfood.com"
              ...
              <Input
                ...
                required
                hint="Le livreur peut changer son mot de passe après la première connexion"
              />
