        ...
        {!sent ? (
          <>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <Button type="submit" size="lg" className="w-full" loading={loading} icon={<ArrowRight className="w-4 h-4" />} iconPosition="right">Send Reset Link</Button>
            </form>
          </>
        ) : (
          <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }}>
            <div className="w-12 h-12 rounded-2xl bg-orbit-success/15 flex items-center justify-center mb-6"><Mail className="w-6 h-6 text-emerald-400" /></div>
            <h1 className="text-2xl font-bold text-slate-100 mb-2">Check your inbox</h1>
            <p className="text-slate-500 text-sm mb-8">We've sent a reset link to your email. It expires in 30 minutes.</p>
            <Button variant="outline" size="lg" className="w-full">Open Email App</Button>
          </motion.div>
        )}
        <Link to="/admin/sign-in" className="flex items-center gap-2 text-xs text-slate-500 hover:text-slate-300 transition-colors mt-8">
